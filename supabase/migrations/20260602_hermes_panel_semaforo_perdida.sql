-- Seguimiento de proformas: el semáforo nunca consideraba el resultado del último
-- seguimiento. Una proforma marcada como "perdida" seguía calculándose por SLA y
-- podía mostrarse como "al día" (verde), atrasada, etc. — y nunca se cerraba como
-- métrica de pérdida.
--
-- Este cambio agrega un estado terminal 'perdida' al semáforo: cuando el último
-- seguimiento tiene estado_resultado = 'perdida', la proforma queda en 'perdida'
-- (deja de pedir seguimientos y deja de contar como "al día"). Así se puede medir
-- cuántas proformas se perdieron.
--
-- hermes_panel_view es una vista materializada: Postgres no soporta CREATE OR
-- REPLACE para matviews, así que la dropeamos y recreamos junto con sus índices y
-- permisos. El refresh final repuebla los datos.
 ...
 SELECT c.numero AS proforma,
    ...
    us.ultimo_seg_estado,
        CASE
            ...
            WHEN COALESCE(us.ultimo_seg_num, 0) >= tr.seguimientos_req THEN NULL::integer
        ...
        CASE
            ...
            WHEN COALESCE(us.ultimo_seg_num, 0) >= tr.seguimientos_req THEN NULL::timestamp without time zone
        ...
        CASE
            ...
            WHEN COALESCE(us.ultimo_seg_num, 0) >= tr.seguimientos_req THEN 'completo'::text
...
-- Índices (recreados tal cual estaban antes del drop)
...
-- Permisos (los mismos que tenía la matview original)
