-- Seguimiento de proformas: el "Completo" automático molestaba.
--
-- Antes, cuando la proforma llegaba a seguimientos_realizados >= seguimientos_req,
-- la vista la marcaba sola como 'completo' y dejaba de permitir más seguimientos.
-- El usuario quiere que el cierre sea SIEMPRE manual (el vendedor la marca vendida
-- o perdida) y poder hacer seguimientos adicionales más allá de los requeridos.
--
-- Cambios sobre hermes_panel_view:
--  1. Se elimina el estado automático 'completo'. Tras cumplir los seguimientos
--     requeridos (sin fecha programada pendiente) la proforma queda en 'al_dia',
--     no se cierra sola.
--  2. proximo_seg_num ya NO se topa en seguimientos_req: sigue incrementando para
--     habilitar seguimientos extra. Solo se corta cuando la proforma se cierra
--     (facturada / vendida / perdida) o no tiene tier.
--  3. Cierre manual: si el último seguimiento es 'ganada' (Vendida) la proforma
--     pasa a 'vendida'; si es 'perdida' pasa a 'perdida'. Ambos son terminales y
--     dejan de pedir seguimientos.
--
-- hermes_panel_view es una vista materializada (no admite CREATE OR REPLACE):
-- se dropea y recrea con sus índices y permisos, y al final se refresca.
...
CREATE MATERIALIZED VIEW public.hermes_panel_view AS
 WITH items_con_costo AS (
         ...
         SELECT items_con_costo.proforma,
            sum(
                CASE
                    WHEN NOT items_con_costo.es_servicio THEN items_con_costo.venta_linea
                    ELSE 0::numeric
                END) AS venta_mercaderia,
            sum(
                CASE
                    WHEN NOT items_con_costo.es_servicio THEN items_con_costo.costo_linea
                    ELSE 0::numeric
                END) AS costo_mercaderia,
            sum(
                CASE
                    WHEN items_con_costo.es_servicio THEN items_con_costo.venta_linea
                    ELSE 0::numeric
                END) AS venta_servicios,
            sum(
                CASE
                    WHEN items_con_costo.sin_costo THEN 1
                    ELSE 0
                END) AS items_sin_costo,
            count(*) AS lineas_total
           FROM items_con_costo
          GROUP BY items_con_costo.proforma
        ), ult_seg AS (
 ...
 SELECT c.numero AS proforma,
    ...
    COALESCE(a.venta_mercaderia, 0::numeric) - COALESCE(a.costo_mercaderia, 0::numeric) AS utilidad_mercaderia,
        CASE
            WHEN COALESCE(a.venta_mercaderia, 0::numeric) > 0::numeric THEN round((a.venta_mercaderia - a.costo_mercaderia) / a.venta_mercaderia * 100::numeric, 2)
            ELSE NULL::numeric
        END AS margen_pct,
