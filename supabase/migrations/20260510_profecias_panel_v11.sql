-- ============================================================
-- Migración: Profecías panel v1.1
-- Fecha: 2026-05-10
-- ============================================================
-- Cambios:
--   1) Columnas vendido_30d/90d/180d: ventas REALES (sin extrapolar)
--   2) velocidad_NNd ahora respeta mínimo de días de vida
--      (NULL para ventanas con muy pocos datos)
--   3) velocidad_ajustada_NNd: excluye facturas atípicas (>3× mediana)
--      cuando el SKU tiene >=5 facturas y mediana >=1
--   4) demanda_proyectada usa velocidad ajustada (con fallback a la
--      bruta) y queda NULL cuando dias_vida<7
--   5) Banderas: datos_insuficientes, tiene_outliers, mediana_factura
-- ============================================================
...
facturas_sku AS (
  ...
  WHERE codigo_interno IS NOT NULL
    ...
    AND COALESCE(cantidad_facturada, 0) > 0
...
outliers_marcados AS (
  SELECT
    ...
    fs.unidades_factura,
    CASE
      WHEN m.num_facturas >= 5
        AND m.mediana_factura >= 1
        AND fs.unidades_factura > 3 * m.mediana_factura
      THEN TRUE
      ELSE FALSE
    END AS es_outlier
  FROM facturas_sku fs
  JOIN medianas m USING (codigo_interno)
),
ventas_agg AS (
  SELECT
    om.codigo_interno,
    MIN(om.fecha_real) AS primera_venta,
    MAX(om.fecha_real) AS ultima_venta,
  ...
  FROM outliers_marcados om
  GROUP BY om.codigo_interno
),
leadtime_dedupe AS (
...
base AS (
  SELECT
    ...
    COALESCE(va.u_aj_30d,  0)::numeric AS u_aj_30d,
    COALESCE(va.u_aj_90d,  0)::numeric AS u_aj_90d,
    COALESCE(va.u_aj_180d, 0)::numeric AS u_aj_180d,
...
calc AS (
  SELECT
    ...
    CASE WHEN b.primera_venta IS NULL THEN 0
         ELSE GREATEST(1, (CURRENT_DATE - b.primera_venta))::int END AS dias_vida_calc
  FROM base b
),
calc_vel AS (
  SELECT
    c.*,
    -- Velocidad bruta (extrapolada) con regla de mínimo días
    CASE
      WHEN c.dias_vida_calc < 7 THEN NULL
      ELSE c.u_30d / GREATEST(LEAST(30, c.dias_vida_calc), 1)::numeric * 30
    END AS velocidad_30d_raw,
    CASE
      WHEN c.dias_vida_calc < 14 THEN NULL
      ELSE c.u_90d / GREATEST(LEAST(90, c.dias_vida_calc), 1)::numeric * 30
    END AS velocidad_90d_raw,
    CASE
      WHEN c.dias_vida_calc < 30 THEN NULL
      ELSE c.u_180d / GREATEST(LEAST(180, c.dias_vida_calc), 1)::numeric * 30
    END AS velocidad_180d_raw,
    -- Velocidad ajustada (sin outliers)
    CASE
      WHEN c.dias_vida_calc < 7 THEN NULL
      ELSE c.u_aj_30d / GREATEST(LEAST(30, c.dias_vida_calc), 1)::numeric * 30
    END AS velocidad_aj_30d_raw,
    CASE
      WHEN c.dias_vida_calc < 14 THEN NULL
      ELSE c.u_aj_90d / GREATEST(LEAST(90, c.dias_vida_calc), 1)::numeric * 30
    END AS velocidad_aj_90d_raw,
    CASE
      WHEN c.dias_vida_calc < 30 THEN NULL
      ELSE c.u_aj_180d / GREATEST(LEAST(180, c.dias_vida_calc), 1)::numeric * 30
    END AS velocidad_aj_180d_raw
  FROM calc c
),
calc2 AS (
  SELECT
    ...
    CASE
      ...
      ELSE 'maduro'
    END AS madurez,
    CASE WHEN c.velocidad_180d_raw > 0
         THEN ROUND(((c.velocidad_90d_raw - c.velocidad_180d_raw) / c.velocidad_180d_raw * 100)::numeric, 2)
         ELSE 0
...
calc3 AS (
  SELECT
    ...
    CASE
      ...
      WHEN c.madurez = 'recien_nacido' THEN
        COALESCE(c.velocidad_aj_30d_raw, c.velocidad_30d_raw) * (SELECT factor_recien_nacido_conservador FROM cfg)
      WHEN c.madurez = 'validacion' THEN
        COALESCE(c.velocidad_aj_90d_raw, c.velocidad_90d_raw, c.velocidad_aj_30d_raw, c.velocidad_30d_raw)
      WHEN c.madurez = 'joven' THEN
        COALESCE(c.velocidad_aj_90d_raw, c.velocidad_90d_raw) * 0.7 +
        COALESCE(c.velocidad_aj_180d_raw, c.velocidad_180d_raw, c.velocidad_aj_90d_raw, c.velocidad_90d_raw) * 0.3
      WHEN c.madurez = 'maduro' THEN
        CASE
          WHEN c.tendencia_pct > 20  THEN
            COALESCE(c.velocidad_aj_90d_raw, c.velocidad_90d_raw) * (1 + LEAST(c.tendencia_pct * 0.005, 0.3))
          WHEN c.tendencia_pct < -20 THEN
            COALESCE(c.velocidad_aj_180d_raw, c.velocidad_180d_raw)
          ELSE
            COALESCE(c.velocidad_aj_90d_raw, c.velocidad_90d_raw) * 0.6 +
            COALESCE(c.velocidad_aj_180d_raw, c.velocidad_180d_raw) * 0.4
        END
    END AS demanda_base
  FROM calc2 c
),
...
calc5 AS (
  SELECT
    c.*,
    CASE WHEN c.demanda_proyectada IS NULL THEN NULL
         ELSE ROUND(((c.lead_time_eff + c.safety_stock_dias) / 30.0 * c.demanda_proyectada)::numeric, 2)
    ...
    CASE
      WHEN c.demanda_proyectada IS NULL OR c.demanda_proyectada = 0 THEN NULL
      ELSE ROUND((c.existencias / c.demanda_proyectada)::numeric, 2)
...
SELECT
  ...
  c.primera_venta,
  c.ultima_venta,
  c.dias_vida_calc                           AS dias_vida,
  ROUND(c.u_30d::numeric, 0)                 AS vendido_30d,
  ROUND(c.u_90d::numeric, 0)                 AS vendido_90d,
  ROUND(c.u_180d::numeric, 0)                AS vendido_180d,
  c.u_30d,
  c.u_90d,
  c.u_180d,
  ...
  c.notas_estado,
  CASE WHEN c.demanda_proyectada IS NULL THEN NULL
       ELSE ROUND(c.demanda_proyectada::numeric, 2)
  END                                        AS demanda_proyectada,
  c.safety_stock_dias,
  c.ciclo_pedido_dias,
  c.punto_reorden,
  c.cantidad_sugerida,
  c.meses_cobertura,
  (c.dias_vida_calc < 7)                     AS datos_insuficientes,
  c.tiene_outliers_90d                       AS tiene_outliers,
  ROUND(c.mediana_factura::numeric, 2)       AS mediana_factura,
  c.num_facturas,
  CASE
