-- Fix: el refresh diario de vistas fallaba intermitente con HTTP 500 en
-- refresh_profecias_panel y bi_recalcular_resumen.
--
-- Causa raíz: estas dos funciones NO traían el `SET statement_timeout = 0`
-- que sí tienen sus hermanas (refresh_mv_consumo_mensual,
-- refresh_mv_items_por_vend_mes). Por eso heredaban el statement_timeout del
-- rol y, cuando el refresh diario coincidía con carga (sync de NEO escribiendo
-- neo_items_facturados —789k filas— + lectores de la app), Postgres las
-- cancelaba por timeout (57014) y PostgREST devolvía 500. Los reintentos del
-- workflow no ayudaban: cada intento caía en la misma ventana lenta.
--
-- Arreglo: agregar `SET statement_timeout TO '0'` a ambas, igual que las otras
-- funciones pesadas de refresh. No cambia ninguna lógica.

CREATE OR REPLACE FUNCTION public.refresh_profecias_panel()
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
