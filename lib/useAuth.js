...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    async function cargarPerfil(userId) {
      // Se acumula el primer fallo real de consulta, pero NO se corta: cada
      // camino (auth_id → email → username → legacy) tiene que poder intentar,
      // como venía andando. Cortar en el primero dejó a todo el equipo afuera
      // con "la base no responde" el 4/9/2026.
      let fallo = null;
      try {
        ...
        // Recién acá se decide si hubo error: si ningún camino trajo perfil Y
        // alguna consulta falló de verdad, fue la base. Si simplemente no está
        // dado de alta, NO es error — es "sin perfil", como siempre.
        setError(perfilLegacy ? null : (fallo || null));
      } catch (e) {
