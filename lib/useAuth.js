...
const PERMISOS_ROL = {
  admin:     ['dashboard','inventario','trazabilidad','rotacion','kronos','reportes','comercial','cif','contenedores','mercado','finanzas','tareas','ponderacion','admin'],
...
export function useAuth() {
  ...
  async function cargarPerfil(userId) {
    const { data } = await supabase.from('genesis_usuarios').select('*').eq('user_id', userId).single();
    setPerfil(data);
    setLoading(false);
  }
