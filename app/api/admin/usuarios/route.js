...
export async function PATCH(req) {
  try {
    ...
    if (nueva_password) {
      const { data: usuario, error: fetchErr } = await getAdmin()
        .from('usuarios_sol')
        .select('auth_id')
        .eq('id', id)
        .single()
      ...
      const { error: authErr } = await getAdmin().auth.admin.updateUserById(
        usuario.auth_id,
...
export async function DELETE(req) {
  try {
    ...
    const { error: delErr } = await getAdmin()
      .from('usuarios_sol')
      .delete()
      .eq('id', id)
    ...
    if (usuario?.auth_id) {
      await getAdmin().auth.admin.deleteUser(usuario.auth_id)
    }
