...
export async function POST(req){
  try{
    ...
    const rows=items.map(i=>({orden_id:orden.id,codigo:String(i.codigo||'').trim(),nombre:String(i.nombre||''),proveedor:String(i.proveedor||''),cantidad_ordenada:parseFloat(i.cantidad)||0,costo_unitario:parseFloat(i.ultimo_costo||i.costo)||0,descuento:parseFloat(i.descuento)||0,dias_tribucion:diasTribucion,cantidad_recibida:0,estado_item:'pendiente',creado_en:ahora}))
