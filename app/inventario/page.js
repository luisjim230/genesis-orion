...
export default function Inventario() {
  ...
  const [dias, setDias] = useState(36);
  ...
  async function cargarDatos() {
    ...
    const { tMap, diasMap } = await calcularTransito(supabase);
    setTransitoMap(tMap);
  ...
  return (
    <div className="module-page glass-module">
      <div className="module-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <button className="btn-outline" onClick={() => { setDatos([]); setCalc([]); cargarDatos(); }}>🔄 Recargar</button>
      </div>
                ...
                <div className="card" style={{ padding: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 200, position: 'relative' }}>
                      ...
                      {showProvSuggestions && nombreOrden.length >= 1 && (() => { const q = nombreOrden.toLowerCase(); const kommoNombres = proveedoresKommo.map(p => p.nombre_proveedor).filter(n => n && !n.startsWith('OC ')); const provDeItems = ordenItems.map(i => i.proveedor || i.ultimo_proveedor || '').filter(Boolean); const todos = [...new Set([...PROVEEDORES_HISTORICOS, ...kommoNombres, ...provDeItems].map(n => n.trim().toUpperCase()))].sort(); return todos.filter(p => p.toLowerCase().includes(q)); })().length > 0 && (
                        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #EAE0E0', borderRadius: 8, zIndex: 999, boxShadow: '0 4px 16px rgba(0,0,0,0.10)', maxHeight: 200, overflowY: 'auto' }}>
                          {[...new Set([...PROVEEDORES_HISTORICOS, ...proveedoresKommo.map(p => p.nombre_proveedor).filter(n => n && !n.startsWith('OC ')), ...ordenItems.map(i => i.proveedor || i.ultimo_proveedor || '').filter(Boolean)].map(n => n.trim().toUpperCase()))].sort().filter(p => p.toLowerCase().includes(nombreOrden.toLowerCase())).map((p, i) => (
