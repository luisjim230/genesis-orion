...
function PanelPermisos({ usuario, onGuardar, onCerrar }) {
  ...
  async function guardarPermisos() {
    ...
    MODULOS.forEach(m => {
      const enRol = rolBaseModulos.includes(m.key)
      if (checks[m.key] !== enRol) nuevosExtra[m.key] = checks[m.key]
    })
    ...
    if (!res.ok || result.error) {
      showMsg('Error: ' + (result.error||'desconocido'), false)
    } else {
  ...
  async function resetPassword() {
    if (!nuevaPass.trim() || nuevaPass.length < 6)
      return showMsg('Mínimo 6 caracteres.', false)
    setSaving(true)
    const res = await fetch('/api/admin/usuarios', {
      method:'PATCH',
      headers:{'Content-Type':'application/json'},
...
export default function AdminPage() {
  ...
  async function crearUsuario() {
    if (!form.password.trim() || !form.nombre.trim())
      return showMsg('Nombre y contraseña son obligatorios.', false)
    if (!form.email.trim() && !form.username.trim())
      return showMsg('Ingresá al menos un email o username.', false)
    setSaving(true)
    try {
    ...
    <div style={{
      ...
      <div style={{ background:SURF, border:`1px solid ${BORDER}`, borderRadius:12, padding:'20px 22px', marginBottom:28 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr', gap:12, marginBottom:14 }}>
          ...
          <div>
            <label style={{ fontSize:'0.72em', color:MUTED, display:'block', marginBottom:4 }}>USERNAME</label>
            <input style={S.input} value={form.username} onChange={e=>setForm({...form,username:e.target.value})} placeholder="luisjim"/>
          </div>
          <div>
            <label style={{ fontSize:'0.72em', color:MUTED, display:'block', marginBottom:4 }}>EMAIL <span style={{color:MUTED,fontWeight:400}}>(opcional)</span></label>
                      ...
                      <tr style={{ background: expPerm ? SURF2 : undefined }}>
                        ...
                        <td style={S.td}>
                          <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                            ...
                            <button onClick={()=>toggleExpandPermisos(u.id)}
                              style={{
