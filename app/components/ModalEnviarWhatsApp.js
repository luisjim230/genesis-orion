...
export default function ModalEnviarWhatsApp({proveedor,items,onClose,onEnviado}){
  ...
  async function enviar(){
    ...
    // Encolar en NEO
    try{
      const res=await fetch('/api/neo/encolar-oc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({proveedor:proveedor||'Sin nombre',items:items.map(i=>({codigo:i.codigo,cantidad:Number(i.cantidad)||0,costo_unitario:Number(i.ultimo_costo||i.costo_unitario||i.precio||0),descuento:Number(i.descuento)||0})),creadoPor:'whatsapp-modal'})})
