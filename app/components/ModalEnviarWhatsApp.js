...
const S={overlay:{position:'fixed',inset:0,background:'rgba(94,39,51,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:99999,padding:'16px'},modal:{background:SURF,borderRadius:14,padding:'24px 28px',width:'100%',maxWidth:520,border:`1px solid ${BORDER}`,boxShadow:'0 8px 32px rgba(94,39,51,0.18)',fontFamily:'DM Sans, sans-serif'},title:{fontSize:'1.05rem',fontWeight:700,color:TEXT,marginBottom:4},sub:{fontSize:'0.82rem',color:MUTED,marginBottom:20},label:{fontSize:'0.75rem',fontWeight:700,color:MUTED,display:'block',marginBottom:5,textTransform:'uppercase',letterSpacing:'0.05em'},input:{width:'100%',background:'#FDF4F4',border:`1px solid ${BORDER}`,borderRadius:8,padding:'9px 12px',fontSize:'0.9rem',color:TEXT,outline:'none',boxSizing:'border-box',fontFamily:'DM Sans, sans-serif'},hint:{fontSize:'0.75rem',color:MUTED,marginTop:4},preview:{background:'#f0f9f0',border:'1px solid #9AE6B4',borderRadius:10,padding:'12px 14px',marginTop:8,fontSize:'0.82rem',color:'#276749',whiteSpace:'pre-wrap',lineHeight:1.5,maxHeight:160,overflowY:'auto'},btnRow:{display:'flex',gap:10,marginTop:20,justifyContent:'flex-end'},btnPrimary:{background:'#25D366',color:'#fff',border:'none',borderRadius:8,padding:'10px 22px',fontWeight:700,cursor:'pointer',fontSize:'0.9rem',fontFamily:'DM Sans, sans-serif'},btnSecondary:{background:'transparent',color:MUTED,border:`1px solid ${BORDER}`,borderRadius:8,padding:'9px 18px',cursor:'pointer',fontSize:'0.88rem',fontFamily:'DM Sans, sans-serif'},itemList:{background:'#FDF4F4',borderRadius:8,padding:'10px 14px',marginTop:8,maxHeight:120,overflowY:'auto',fontSize:'0.82rem',color:TEXT}}
    ...
    <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={S.modal}>
        ...
        <div style={{marginTop:16}}>
          ...
          <input style={S.input} type="tel" placeholder="50688887777 (código país + número)" value={cargando?'Cargando...':telefono} onChange={e=>setTelefono(e.target.value)} disabled={cargando} autoFocus/>
        ...
        <div style={S.btnRow}>
          ...
          <button style={S.btnPrimary} onClick={enviar}>📱 Abrir WhatsApp</button>
        </div>
      </div>
    </div>
  )
