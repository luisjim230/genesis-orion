"""
neo_antiguedad_proveedores_downloader.py — Descarga "Antigüedad de saldos — Proveedores"
de NEO y sube a Supabase (tabla fin_cuentas_pagar).
...
Cómo correr manualmente:
  ...
  python3 neo_antiguedad_proveedores_downloader.py
...
TABLA       = "fin_cuentas_pagar"

DOWNLOAD_DIR = BASE / "downloads"
DOWNLOAD_DIR.mkdir(exist_ok=True)
LOG_FILE     = BASE / "neo-antiguedad-proveedores.log"
...
COL_MAP = {
    ...
    norm("Tipo"):                    "tipo",
    norm("Número"):                  "numero",
    norm("Fecha de la compra"):      "fecha_compra",
    norm("Fecha de vencimiento"):    "fecha_vencimiento",
    norm("Saldo original"):          "saldo_original",
    norm("Pagos aplicados"):         "pagos_aplicados",
    norm("Notas aplicadas"):         "notas_aplicadas",
    norm("Saldo actual"):            "saldo_actual",
    norm("Moneda"):                  "moneda",
    norm("Sin vencer"):              "sin_vencer",
    norm("1 - 8 Días"):              "dias_1_8",
    norm("9 - 15 Días"):             "dias_9_15",
    norm("16 - 22 Días"):            "dias_16_22",
    norm("23 - 30 Días"):            "dias_23_30",
...
async def descargar():
    ...
    async with async_playwright() as p:
        ...
        # ── Navegar: Cuentas por pagar → Antigüedad de saldos ─────────────────
        await page.locator("#mostrar_barra_izquierda").click()
        await page.get_by_role("link", name="Cuentas por pagar").click()
        ...
        await page.wait_for_load_state("networkidle")
        log.info("✅ Antigüedad de saldos cargado")

        # ── Configurar reporte: Ordenar por código, modo detallado ────────────
...
def subir_a_supabase(excel_path):
    ...
    # Buscar la fila de encabezados (primera que tenga "Código" en columna 0)
    ...
    if header_row is None:
        log.error("❌ No se encontró fila con 'Código' — abortando")
        return False
    ...
    if "codigo" not in cols:
        log.error("❌ No se encontró columna 'Código' — abortando")
        return False

    df = df[cols].copy()

    # Filtrar filas vacías, totales generales y subtotales por proveedor
    # Los subtotales no tienen fecha_compra — solo las filas de detalle real la tienen
    df = df.dropna(subset=["codigo"])
    df = df[df["codigo"].astype(str).str.strip() != ""]
    df = df[~df["codigo"].astype(str).str.startswith("Total")]
    df = df[df["fecha_compra"].notna()]  # excluir subtotales por proveedor
    ...
    # Actualizar sync_status para el badge en SOL
    ahora = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
    supa_request("PATCH", "sync_status?id=eq.antiguedad_proveedores", {
        "ultima_sync": ahora,
        "exitoso": exito,
        "updated_at": ahora,
    })
...
async def main():
    ...
    tomar_candado_neo("antiguedad_proveedores", log)
    log.info("=" * 50)
    log.info(f"NEO → Antigüedad saldos proveedores  [{datetime.now():%Y-%m-%d %H:%M}]")
    log.info("=" * 50)
