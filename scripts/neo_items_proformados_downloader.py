...
def rango_30_dias():
    """Últimos 30 días corridos hasta hoy."""
...
def parse_y_subir(excel_path):
    ...
    if en_rango == 0:
        ...
        alerta_telegram(f"⚠️ <b>Ítems proformados</b>\n{total} líneas pero ninguna en los últimos 30 días.")

    if total < UMBRAL_MIN:
        log.warning(f"⚠️ Solo {total} líneas — sospechoso")
        alerta_telegram(f"⚠️ <b>Ítems proformados</b>\nSolo {total} líneas (umbral {UMBRAL_MIN}).")

    log.info(f"Llamando RPC hermes_upsert_proformas_items con {total} líneas ({en_rango} en últimos 30 días)...")
...
def main():
    ...
    tomar_candado_neo("proformas_items", log)
    log.info("=" * 60)
